...
export class CanTextParserService {
  ...
  replaceKeyWithValueInDynamicTextOrJSON<T>(
    ...
    if (separatedKeys && separatedKeys.length > 0) {
      separatedKeys.forEach(sKey => {
        ...
        const mappedValue = `${mappedKey} : ${mappedData[mappedKey]}`;
   ...
   * @param text: string
   */
    replaceKeyWithValueInDynamicTextOrJSONForGpt<T>(
      data: { [key: string]: any },
      text: string,
      keys: {}
    ): T {
      ...
      if (separatedKeys && separatedKeys.length > 0) {
        separatedKeys.forEach(sKey => {
          const mappedKey = this.mapSeparatedKey(sKey);
          const mappedValue = `${keys[mappedKey]} : ${mappedData[mappedKey]}`;
          mappedText = mappedText.replace(sKey, mappedValue);
        });
        try {
          // if JSON string return Object
   ...
   * @param text: string
   */
  getKeysFromDynamicText(text: string) {
    // Separator
